...
import { useMisReservas, useCancelReserva } from '@/hooks/use-prestamos';
...
import { Loader2, Bookmark, Trash } from 'lucide-react';

export default function MisReservasPage() {
  const [page, setPage] = useState(1);
  const { data, isLoading } = useMisReservas({ page, pageSize: 10 });
  ...
  return (
    <div className="flex-1 w-full max-w-4xl mx-auto p-4 md:p-6 space-y-6">
      <div>
        ...
        <p className="text-zinc-500 text-sm">Ejemplares que has reservado</p>
      </div>
                ...
                {(!data?.data || data.data.length === 0) ? (
                  ...
                  data.data.map((r) => (
                    <TableRow key={r.idReserva} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30">
                      ...
                      <TableCell className="font-medium text-zinc-900 dark:text-zinc-50">{r.edicionVolumen?.libro?.titulo || '-'}</TableCell>
