...
export default function ProveedoresPage() {
  ...
  const { data, isLoading, refetch, isFetching } = useProveedores({ page, pageSize: 10 });
  ...
  return (
    <div className="flex-1 w-full max-w-6xl mx-auto p-4 md:p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-50">Proveedores</h1>
          <p className="text-zinc-500 text-sm">Gestión de proveedores</p>
        </div>
      ...
      {isLoading ? (
        ...
        <>
          <ProveedorTable proveedores={data?.data || []} />
          {totalPages > 1 && (
