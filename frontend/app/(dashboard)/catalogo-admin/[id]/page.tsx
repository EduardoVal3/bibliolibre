...
export default function LibroDetailPage() {
  ...
  const updateCopyMutation = useMutation({
    mutationFn: ({
      ...
      copyId: number;
      data: { estadoFisico?: string; disponibilidad?: string };
    ...
    return (
      <div className="max-w-2xl mx-auto p-6 text-center space-y-4">
        ...
        <p className="text-zinc-500">
          El libro solicitado no existe o no se pudo cargar.
        </p>
  ...
  return (
    <div className="flex-1 w-full max-w-5xl mx-auto p-4 md:p-6 space-y-6">
      <Link
        href="/catalogo-admin"
        className="inline-flex items-center gap-1.5 text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors text-sm font-medium"
      >
        ...
        <Card className="md:col-span-2 bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
          <CardHeader className="p-6 border-b border-zinc-100 dark:border-zinc-800">
            <div className="flex justify-between items-start gap-4">
              <div>
                <span className="text-xs font-semibold text-indigo-550 uppercase tracking-wider">
                  {libro.categoria?.nombreCategoria}
                </span>
                <CardTitle className="text-2xl font-bold mt-1 text-zinc-900 dark:text-zinc-50">
                  {libro.titulo}
                </CardTitle>
              </div>
              <Link href={`/catalogo-admin/editar/${libro.idLibro}`}>
                <Button
                  variant="outline"
                  size="sm"
              ...
              <div className="col-span-2">
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {libro.palabrasClave?.map((pc) => (
                    <span
                      key={pc.idPalabraClave}
                      className="px-2 py-0.5 text-xs bg-zinc-100 text-zinc-705 dark:bg-zinc-800 dark:text-zinc-300 rounded-md"
                    >
                      {pc.palabra}
                    </span>
                  ))}
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          <Button
            ...
            className="h-8 px-3 gap-1 bg-indigo-600 hover:bg-indigo-700 text-white font-medium cursor-pointer border-0"
          >
              ...
              {!libro.ejemplares || libro.ejemplares.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-zinc-400">
                    Este libro no cuenta con ningún ejemplar registrado en
                    inventario.
                  </td>
                </tr>
              ) : (
                  ...
                  >
                    ...
                    <td className="p-4 text-right">
                      <div className="flex justify-end gap-1.5">
                        <ReservaButton
                          idEdicionVolumen={ejemplar.idEdicionVolumen}
                          disabled={ejemplar.disponibilidad !== "Disponible"}
                        />
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleEditCopyClick(ejemplar)}
        ...
        <DialogContent className="sm:max-w-md bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800">
          <form onSubmit={handleAddCopySubmit}>
                ...
                >
                  ...
                  {locations?.map((loc) => (
                    <option key={loc.idUbicacion} value={loc.idUbicacion}>
                      Sección {loc.seccion}, Pasillo {loc.pasillo}, Estantería{" "}
                      {loc.estanteria}
                    </option>
                  ))}
                </NativeSelect>
            ...
            <DialogFooter>
              ...
              <Button
                type="submit"
                className="bg-indigo-600 hover:bg-indigo-700 text-white font-medium border-0 cursor-pointer"
