...
import { useRecurso, useUpdateRecurso } from '@/hooks/use-recursos-digitales';
...
export default function EditarRecursoPage() {
  ...
  const { data: recurso, isLoading } = useRecurso(Number(id));
  ...
  return (
    <div className="flex-1 w-full max-w-2xl mx-auto p-4 md:p-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-50">Editar Recurso</h1>
          <p className="text-zinc-500 text-sm">{recurso?.titulo}</p>
        </div>
      </div>
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm">
        <RecursoDigitalForm
          initialValues={recurso}
          onSubmit={(values) => updateMutation.mutate({ id: Number(id), data: values }, { onSuccess: () => router.push('/biblioteca-digital') })}
