    ...
    <Card className="flex flex-col h-full bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
      <CardHeader className="p-5 pb-3">
        <div className="flex justify-between items-start gap-4 mb-2">
          <Badge variant="secondary" className="bg-zinc-100 text-zinc-800 dark:bg-zinc-800 dark:text-zinc-300 font-medium">
            {libro.categoria}
          </Badge>
      ...
      <CardContent className="px-5 py-0 flex-1 flex flex-col justify-between">
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-zinc-400">
            <span>Edición: <strong className="text-zinc-600 dark:text-zinc-300">{libro.edicion}</strong></span>
            <span>Año: <strong className="text-zinc-600 dark:text-zinc-300">{libro.anioPublicacion}</strong></span>
          </div>
        </div>
      ...
      <CardFooter className="p-5 pt-0 flex justify-between items-center gap-3">
        ...
        <Link href={`/catalogo/${libro.idLibro}`} passHref>
          <Button size="sm" variant="outline" className="border-zinc-300 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-800 font-medium">
            Ver Detalles
          </Button>
        </Link>
      </CardFooter>
