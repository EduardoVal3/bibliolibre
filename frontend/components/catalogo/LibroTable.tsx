...
export function LibroTable({ libros }: LibroTableProps) {
  ...
  const handleDelete = (id: number, titulo: string) => {
    if (confirm(`¿Estás seguro de que deseas eliminar el libro "${titulo}" y todos sus ejemplares?`)) {
      ...
      <Table>
        ...
        <TableBody>
          {libros.length === 0 ? (
            ...
            libros.map((libro) => (
              <TableRow key={libro.idLibro} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30">
                ...
                <TableCell className="text-center">
                  <span className={`font-semibold ${Number(libro.ejemplaresDisponibles) > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}`}>
                    {libro.ejemplaresDisponibles}
                  </span>
                </TableCell>
                <TableCell className="text-right">
                  <div className="flex justify-end gap-1.5">
                    <Link href={`/catalogo/${libro.idLibro}`} passHref>
                      <Button size="icon" variant="ghost" className="h-8 w-8 hover:bg-zinc-100 dark:hover:bg-zinc-800">
                        <Eye className="w-4 h-4 text-zinc-500" />
                      </Button>
                    </Link>
                    <Link href={`/catalogo/editar/${libro.idLibro}`} passHref>
                      <Button size="icon" variant="ghost" className="h-8 w-8 hover:bg-zinc-100 dark:hover:bg-zinc-800">
